...
def item_mod():
    global player,opponent,ball_speed_x,item,starting_opponent_paddle,starting_player_paddle
    if ball.colliderect(item):
        item = pygame.Rect(random.choice((20,600)),random.choice((20,460)),20,20)
        if ball_speed_x > 0:
            opponent = pygame.Rect(opponent.x, opponent.y,15,starting_opponent_paddle + 20)
            player = pygame.Rect(player.x, player.y,15,starting_player_paddle - 20)
            print("Opponent Hit")
        if ball_speed_x < 0:
            player = pygame.Rect(player.x, player.y,15,starting_player_paddle + 20)
            opponent = pygame.Rect(opponent.x, opponent.y,15,starting_opponent_paddle - 20)
...
def ball_restart():
    global ball_speed_x,ball_speed_y,score_time,player,opponent,item
    ...
    if current_time - score_time < 2100:
        ...
        item = pygame.Rect(random.choice((200,400)),random.choice((200,400)),20,20)
        score_time = None            
...
pygame.display.set_caption("Pong - Mod Template")

# Game Rectangles
starting_player_paddle = 100
starting_opponent_paddle = 100
ball = pygame.Rect(screen_width/2 - 10,screen_height/2 - 10,20,20)
player = pygame.Rect(screen_width - 20, screen_height/2 - 70,15,starting_player_paddle)
opponent = pygame.Rect(10, screen_height/2 - 70,15,starting_opponent_paddle)
item = pygame.Rect(random.choice((200,400)),random.choice((200,400)),20,20)
